...
apartments.sf %>% 
  ...
  geom_sf(data = apartments.sf, aes(color = q5(price))) +
...
set.seed(717)
reg1.log.cv <- 
  train(logprice ~ ., data = st_drop_geometry(dflog_reg) %>% 
          dplyr::select(all_of(reg.vars.log)), 
        method = "lm", 
        trControl = fitControl, 
        na.action = na.pass)
...
#Standard Deviation and Histogram of MAE
reg1.log.cv.resample <- reg1.log.cv$resample

sd(reg1.log.cv.resample$MAE)

ggplot(reg1.log.cv.resample, aes(x=MAE)) + geom_histogram(color = "grey40", fill = "#232761ff", bins = 50) + 
  labs(title="Histogram of Mean Average Error Across 100 Folds") +
  plotTheme()
